...
function HeroSection() {
    ...
    return ( 
        <div className="relative w-full h-screen overflow-hidden">
            ...
            <div className='absolute bottom-0 left-1/4 z-10 flex flex-row items-center justify-between w-1/2 bg-white border rounded-3xl border-black translate-y-[50px]'>
                {names.map((item)=>(
                    <div key={item.title1} className='flex flex-col items-center mb-4 ml-10 mr-10 mt-5'>
                        {/* <img src={item.src} alt={item.alt} /> */}
                        <Image src={item.src} alt={item.alt} width={item.width} height={item.height} />
                        <h3 className="mt-2 text-lg font-semibold text-black">{item.title1}</h3>
                        <h3 className="text-lg font-semibold text-black">{item.title2}</h3>
                    </div>
                ))}  
            </div>
        </div>
